...
try {
  ...
  const pages = ["index.html", "wishlist.html", "footprints.html", "exercise.html", "timeline.html", "notes.html", "gifts.html", "data-center.html", "game-hall.html", "game.html", "lyrics.html", "timer.html", "sync.html", "memory.html", "world.html", "province.html", "literature.html", "pi-memory.html", "clue-guess.html", "speed-quiz.html", "achievements.html"];
  ...
  await click('.place-card [data-action=edit]');await evaluate("document.querySelector('#place-status').value='special';document.querySelector('#place-form').requestSubmit()");await click('[data-filter=special]');await assert(await evaluate("document.querySelectorAll('.place-card').length===1&&document.querySelector('#visited-count').textContent==='1'"),"travel edit or filter failed");
  await click('.place-card [data-action=edit]');await evaluate("document.querySelector('#place-status').value='wanted';document.querySelector('#place-form').requestSubmit()");await click('[data-filter=all]');await click('#random-next');await assert(await evaluate("document.querySelector('#next-dialog').open&&document.querySelector('#next-title').textContent==='杭州'"),"random next stop failed");await click('#next-dialog .random-close');
  await evaluate("window.confirm=()=>false");await click('.place-card [data-action=delete]');await assert(await evaluate("JSON.parse(localStorage.getItem('xiaoluXiaogTravelV1')).records.length===1"),"travel cancelled deletion failed");await evaluate("window.confirm=()=>true");await click('.place-card [data-action=delete]');await assert(await evaluate("JSON.parse(localStorage.getItem('xiaoluXiaogTravelV1')).records.length===0"),"travel confirmed deletion failed");
  await click('[data-view=transport]');await assert(await evaluate("document.querySelectorAll('.transport-card').length===25&&document.querySelector('#transport-count').textContent.includes('0 / 总计 25')"),"transport catalog count failed");await click('.transport-card[data-id=subway] [data-action=transport-toggle]');await assert(await evaluate("JSON.parse(localStorage.getItem('xiaoluXiaogTransportV1')).entries.subway.state==='experienced'&&document.querySelector('#transport-percent').textContent==='4%'"),"transport unlock or progress failed");
  await click('.transport-card[data-id=subway] [data-action=transport-edit]');await evaluate("document.querySelector('#transport-state').value='special';document.querySelector('#transport-place').value='上海';document.querySelector('#transport-story').value='第一次一起坐地铁';document.querySelector('#transport-times').value='3';document.querySelector('#transport-form').requestSubmit()");await assert(await evaluate("JSON.parse(localStorage.getItem('xiaoluXiaogTransportV1')).entries.subway.state==='special'&&JSON.parse(localStorage.getItem('xiaoluXiaogTransportV1')).entries.subway.times===3"),"transport details edit failed");
  await click('#add-transport');await evaluate("document.querySelector('#transport-name').value='雪地摩托';document.querySelector('#transport-icon').value='❄️';document.querySelector('#transport-category').value='future';document.querySelector('#transport-form').requestSubmit()");await assert(await evaluate("JSON.parse(localStorage.getItem('xiaoluXiaogTransportV1')).custom.some(x=>x.name==='雪地摩托')&&document.querySelectorAll('.transport-card').length===26"),"custom transport failed");await open("footprints.html",390);await click('[data-view=transport]');await assert(await evaluate("document.querySelector('#transport-count').textContent.includes('2 / 总计 26')&&document.querySelector('.mini-achievement.unlocked strong').textContent==='城市探索者'&&localStorage.getItem('v19Sentinel')==='keep-me'"),"transport persistence, achievement, or storage isolation failed");
  await evaluate("localStorage.setItem('xiaoluXiaogTravelV1','{bad');localStorage.setItem('xiaoluXiaogTransportV1','{bad')");await open("footprints.html",390);await assert(await evaluate("document.querySelector('.empty-footprint')&&document.querySelectorAll('.transport-card').length===25"),"malformed storage fallback failed");
  console.log("PASS v1.9 travel CRUD/filter/random; transport catalog/custom/progress/achievements; storage safety");
  await open("literature.html", 390); await click('[data-lit-mode="classics"]');await assert(await evaluate("!document.querySelector('#classics-setup').hidden && document.querySelectorAll('[data-classics-mode]').length===5"),"classics mode selector failed");await click('[data-classics-mode="综合挑战"]');
  ...
  for(const book of ["红楼梦","西游记","三国演义","水浒传"]){await open("literature.html",390);await click('[data-lit-mode="classics"]');await click(`[data-classics-mode="${book}"]`);await assert(await evaluate(`LiteratureData.classics.find(q=>q.q===document.querySelector('.challenge-question').textContent).book===${JSON.stringify(book)}`),`${book} special draw failed`)}
  await open("literature.html",390);await click('[data-lit-mode="poetry"]');
  for(let i=0;i<10;i++){await evaluate(`(()=>{const prompt=document.querySelector('.poetry-prompt strong').textContent,reverse=document.querySelector('.poetry-prompt small').textContent.includes('上一句'),q=LiteratureData.poetry.find(x=>reverse?x.after===prompt:x.before===prompt),answer=reverse?q.before:q.after;document.querySelector('#poetry-input').value='  '+answer+'。 ';document.querySelector('#poetry-form').requestSubmit()})()`);await click('#lit-next');}
  ...
  console.log(`PASS responsive: ${pages.length} pages × 8 viewports; 11 game entries; wishlist regression; 7 original games; 35 achievements; v2.5 data center and life links`);
