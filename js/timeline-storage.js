((root, factory) => {
  ...
  "use strict";
  const KEY = "xiaoluXiaogTimelineV1", SCHEMA_VERSION = 1;
  const SOURCE_MODULES = ["manual", "wishlist", "footprints", "achievement", "notes", "gift", "project"];
  ...
  function normalizeMemory(input = {}) {
    const sourceModule = SOURCE_MODULES.includes(input.source?.module) ? input.source.module : "manual";
    ...
    return {
      ...
      source: { module: sourceModule, itemId: text(input.source?.itemId, 100), label: text(input.source?.label, 120) },
      links: { footprintIds: stringIds(links.footprintIds), wishIds: stringIds(links.wishIds), achievementIds: stringIds(links.achievementIds), noteIds: stringIds(links.noteIds), giftIds: stringIds(links.giftIds), projectIds: stringIds(links.projectIds) },
