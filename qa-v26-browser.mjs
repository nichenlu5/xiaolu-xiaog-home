...
try {
  ...
  await assert(await evaluate("document.querySelector('#journey-date-range').textContent.includes('2026年9月14日')&&document.querySelector('#journey-date-range').textContent.includes('2029年6月（预计）')"), "exact admission and month-only graduation dates were not rendered");
  ...
  await open("index.html", 390); await assert(await evaluate("document.querySelector('.graduate-summary-card')&&document.querySelector('#home-journey-elapsed').textContent.includes('已读研')&&document.querySelector('#home-journey-percent').textContent.endsWith('%')"), "home journey summary failed");
  console.log("PASS v2.6 browser: settings, exact admission date, milestone CRUD/filter and synchronized home journey summary");
